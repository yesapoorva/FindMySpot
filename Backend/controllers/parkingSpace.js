...
const nearestParkingSpace = async (req, res) => {
  ...
  try {
    ...
    const allParkingSpaces = await ParkingSpace.find({ status: 'Available' }, 'name availability');
    ...
    const nearbyParkingSpaces = allParkingSpaces.filter((parkingSpace) => {
      ...
      console.log(`Parking Space ${parkingSpace.name}: Distance - ${distance} kilometers`);
